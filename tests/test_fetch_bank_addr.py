...
def _machine(lanes: int):
    ...
    config = gen_o3_rv32im_config(fe_lanes=lanes, commit_lanes=lanes)
...
def test_the_memory_states_the_bank_as_a_second_address_region():
    config = gen_o3_rv32im_config(fe_lanes=4, commit_lanes=4)
...
def test_a_port_that_cannot_report_valid_is_refused():
    ...
    config = gen_o3_rv32im_config(fe_lanes=1, commit_lanes=1)
...
def test_the_pc_resets_to_the_isas_reset_vector_in_the_emitted_verilog(tmp_path):
    ...
    config = dataclasses.replace(gen_o3_rv32im_config(fe_lanes=2, commit_lanes=2),
