...
#   regs.py    — architectural register classes (§6.1); x only, PC is not one
#   ops.py     — op vocabulary + the machine's execution units (§1.2)
...
#   rv32i.py   — the per-format µop shapes (§6.4), the Mop table, and the
#                IsaBase assembly
...
# is one too: `regs.RegFile`, built by `x_file()` and shared by every shape
...
from .regs import ImmTarget, RegFile, X_LEN, x_file
