...
class EasyMem(MemBase):
    ...
    def com_declare(self):
        ...
        self.bank_index = [wire(self.index_width, f"bank{k}_index")
                           for k in range(self.bank_cnt)]
        self.bank_data  = [mem_ele(self.banks[k], self.bank_index[k],
                                   data_bits, True, f"bank{k}_out")
                           for k in range(self.bank_cnt)]
        self.bank_write = [mem_ele(self.banks[k], self.bank_index[k],
                                   data_bits, False, f"bank{k}_in")
                           for k in range(self.bank_cnt)]
    ...
    def gen_read_port(self, name: Optional[str] = None) -> MemPortReadValid:
        ...
        return MemPortReadValid(self.addr_meta, parts["addr"], parts["meta"],
                                PortTiming.LEVEL, parts["data"], parts["valid"])
    ...
    def gen_write_port(self, name: Optional[str] = None) -> MemPortWrite:
        ...
        return MemPortWrite(self.addr_meta, parts["addr"], parts["meta"],
                            PortTiming.NEXT_EDGE, parts["data"],
                            parts["enable"])
    ...
    def route_access(self):
        ...
        for bank in range(self.bank_cnt):
            self._drive_bank_index(bank)
            self._drive_bank_write(bank)
        for port in self._reads:
            self._drive_read_answer(port)

    def _wants(self, port: dict, bank: int):
        """Whether this port named this bank — always true when unbanked."""
        if port["bank"] is None:
            return val(1, 1)
        return port["bank"] == bank

    def _drive_bank_index(self, bank: int) -> None:
        ...
        for port in reversed(self._reads):
            chosen = (port["index"] if chosen is None
                      else mux(self._wants(port, bank), port["index"], chosen))
        for port in self._writes:                   # a write claims it outright
            chosen = (port["index"] if chosen is None else
                      mux(self._wants(port, bank) & port["enable"],
                          port["index"], chosen))
        if chosen is not None:
            self.bank_index[bank] *= chosen

    def _drive_bank_write(self, bank: int) -> None:
        for port in self._writes:
            with zif(self._wants(port, bank) & port["enable"]):
                self.bank_write[bank] |= port["data"]
    ...
    def _drive_read_answer(self, port: dict) -> None:
        ...
        for bank in range(1, self.bank_cnt):
            picked = mux(self._wants(port, bank), self.bank_data[bank], picked)
        port["data"] *= picked
